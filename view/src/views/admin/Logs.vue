    ...
    <div class="search-bar">
      <a-form layout="inline">
        ...
        <a-form-item class="search-button">
          <a-button type="primary" :disabled="!dateRange.length && !username && !ip" @click="handleSearch">搜索</a-button>
...
const fetchLogs = async () => {
  loading.value = true
  try {
    const { data } = await axios.post('/api/admin/logs', qs.stringify({
      page: current.value,
...
const fetchStats = async () => {
  try {
    const { data } = await axios.post('/api/admin/logs/stats', qs.stringify({
      startDate: dateRange.value[0]?.format('YYYY-MM-DD'),
      endDate: dateRange.value[1]?.format('YYYY-MM-DD')
    }))
    initCharts(data)
