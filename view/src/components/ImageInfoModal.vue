  ...
  >
    <div class="modal-image-container">
      <a-image
        class="modal-image"
        placeholder
        :src="imageInfo.type === 'local' ? userStore.config.site.url + imageInfo.url : imageInfo.url"
        :preview="false"
      />
    </div>
    ...
    <div
      v-if="images.length > 1 && images.length > imageKey + 1"
...
<script setup>
  ...
  const goUserPage = (imageInfo, type) => {
    ...
    router.push(
      type === 'user' ? `/user/${imageInfo.user._id}` : `/user/${imageInfo.user._id}/album/${imageInfo.album._id}`
    )
  }
</script>
